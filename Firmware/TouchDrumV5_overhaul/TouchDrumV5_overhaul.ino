...
const int channel = 1;
int midiNote[] = {60,61,62,63,64,65,66,67};
...
void touchCheck()
{
  ...
  { 
    if (touchRead(pinTouch[i]) > touchThreshold[i])
    {
      ...
      if(midiNoteFlag[i] == 0)
      {
        usbMIDI.sendNoteOn(midiNote[i], 127, channel);  // 60 = C4
        midiNoteFlag[i] = 1;
      }
    ...
    else if(touchRead(pinTouch[i]) <= touchThreshold[i]) 
    {
      ...
      if(midiNoteFlag[i] == 1)
      {
      usbMIDI.sendNoteOff(midiNote[i], 127, channel);
      midiNoteFlag[i] = 0;
      }  
    }   
  }//end for loop

    if (touchRead(pinTouch[8]) > touchThreshold[8])
    {
    note_index++;
    padNumber++;
    //check for overflow
    ...
    if (touchRead(pinTouch[9]) > touchThreshold[9])
    {
